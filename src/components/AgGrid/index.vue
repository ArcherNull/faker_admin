...
<template>
  <div class="AgGrid">
    ...
    <!-- :style="style"，表格样式 ； class ， 表格主题 ；:grid-options="$agTableOptions.gridOptions"表格配置项；:default-col-def="$agTableOptions.defaultColDef"全局列配置  -->
    ...
    <!--     <ag-grid-vue
      ...
      :default-col-def="$agTableOptions.defaultColDef"
      :column-defs="$agTableOptions.data.columnDefs"
      :row-data="rowData"
      :row-height="30"
      :header-height="30"
      edit-type="fullRow"
      ...
      @columnPinnedCountChanged='onColumnPinnedCountChanged'
...
// 引入ag-grid-vue 主题样式
import 'ag-grid-community/dist/styles/ag-grid.css' // 基础表格样式

import 'ag-grid-community/dist/styles/ag-theme-balham.css' // balham
import 'ag-grid-community/dist/styles/ag-theme-balham-dark.css' // balham-dark

import 'ag-grid-community/dist/styles/ag-theme-alpine.css' // alpine
import 'ag-grid-community/dist/styles/ag-theme-alpine-dark.css' // alpine-dark

import 'ag-grid-community/dist/styles/ag-theme-material.css' // material

import 'ag-grid-community/dist/styles/ag-theme-blue.css' // blue

import 'ag-grid-community/dist/styles/ag-theme-bootstrap.css' // bootstrap

import 'ag-grid-community/dist/styles/ag-theme-fresh.css' // refresh

import 'ag-grid-community/dist/styles/ag-theme-dark.css' // dark

import { agGridMethods } from './common/agGrid-methods.js'
...
export default {
  ...
  data () {
    return {
      ...
      // ag-grid表格配置项-- 用户自定义配置型
      agGridOptions: {
        id: 'agTableid',
        style: {
          width: '100%'
        },
        // 主题可设置值为： alpine / balham
        theme: '',
  ...
  computed: {
    ...
    agGridDefaultOptions () {
      ...
      const agGridLoading = `
        <span>
          <div class="MyLoading">
            <span class="loader-${this.currentSelectedLoadingType}"></span>
          </div>
          <div>${wellKnownSaying()}</div>
        </span>`

      // 顶部固定行样式
      // const pinnedTopRowDataStyle = {
      //   'font-weight': 'bold',
      //   color: 'rgba(0,0,0,0.01)'
      // }

      // 底部固定行样式
      const pinnedBottomRowDataStyle = {
        'font-weight': 'bold',
        color: 'rgba(0,0,0,0.01)'
      }
      ...
      const agTableConfig = {
        ...
        gridOptions: {
          defaultColDef: {
            ...
            width: this.width,
            minWidth: this.minWidth, // 【重要】最小宽度， 最小和最大宽度可限制列拖动
            maxWidth: this.maxWidth, // 【重要】最大宽度
            editable: this.editable, // 是否可编辑,存在突然失效的情况【未知其原因】在键盘中按下D可打开编辑,跟options中的singleClickEdit: false,true单击可编辑存在关联,false不实现单击可关联
            suppressMenu: this.suppressMenu, // 【重要】列头右侧的菜单栏按钮显示和隐藏，true为隐藏，false为显示
            cellStyle: this.cellStyle, // 列的单元格默认样式
            hide: false, // 是否隐藏
            sort: this.sort, // 前端排序方式 asc 顺序 desc倒序，还有自定义拍序 .优先顺序为 sortable为true,才能设置排序方法，sort与 comparator 不能共存
            // comparator: dateComparator, // 自定义排序，dateComparator为排序方法名
            sortable: this.sortable, // 是否开启排序，这个是展示原本的列头排序，当sort设置不是true时，有数字展示；操作之后就消失
            resizable: this.resizable, // true可以拖动改变列的大小，false不允许用户拖动改变列大小
            filter: this.filter, // 是否显示筛选
            // flex: 1, // 列平分
            // 【未知其作用】
          ...
          floatingFilter: true, // 【重要】浮动过滤器的显示和隐藏，就是列头下面的筛选器
      ...
      fileName: '导出数据.csv'
          ...
          getRowStyle: function (params) {
            if (params.node.rowPinned) {
              return pinnedBottomRowDataStyle
            }
          },
          pinnedBottomRowData: [],
          // 底部统计框自定义组件
